...
# https://github.com/cartesi/rollups/blob/main/onchain/rollups/contracts/facets/ERC721PortalFacet.sol.
ERC721_TRANSFER_HEADER = b"Y\xda*\x98N\x16Z\xe4H|\x99\xe5\xd1\xdc\xa7\xe0L\x8a\x990\x1b\xe6\xbc\t)2\xcb]\x7f\x03Cx"  # TODO
# Function selector to be called during the execution of a voucher that transfers funds,
# which corresponds to the first 4 bytes of the Keccak256-encoded result of "safeTransferFrom(address,address,uint256,bytes)",
...
def handle_advance(data):
    ...
    try:
        ...
        # Attempt to decode input as an ABI-encoded ERC-721 deposit
        ...
        try:
            ...
            msg = "Payload does not conform to ERC-721 deposit ABI"
            logger.error(f"{msg}\n{traceback.format_exc()}")
            return reject_input(msg, data["payload"])

        # Check if the header matches the Keccak256-encoded string "ERC721_Transfer"
        input_header = decoded[0]
        if input_header != ERC721_TRANSFER_HEADER:
            return reject_input(
                f"Input header is not from an ERC-721 transfer", data["payload"]
            )
        ...
        amount = decoded[3]
        notice_str = (
            f"Deposit received from: {depositor}; ERC-721: {erc721}; Amount: {amount}"
        )
        ...
        transfer_payload = TRANSFER_FUNCTION_SELECTOR + encode_abi(
            ["address", "uint256"], [depositor, amount]
        )
        # Post voucher executing the transfer on the ERC-721 contract: "I don't want your money!"
        voucher = {"address": erc721, "payload": "0x" + transfer_payload.hex()}
