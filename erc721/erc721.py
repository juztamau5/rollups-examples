...
def handle_advance(data):
    ...
    try:
        ...
        if input_header != ERC721_TRANSFER_HEADER:
            return reject_input(f"Input header is not from an ERC-721 transfer", data["payload"])

        # Post notice about the deposit
        depositor = decoded[1]
        erc721 = decoded[2]
        amount = decoded[3]
        notice_str = f"Deposit received from: {depositor}; ERC-721: {erc721}; Amount: {amount}"
