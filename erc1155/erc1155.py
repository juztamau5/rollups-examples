...
# Default header for ERC-1155 transfers coming from the Portal, which corresponds
# to the Keccak256-encoded string "ERC1155_Transfer", as defined at
# https://github.com/cartesi/rollups/blob/main/onchain/rollups/contracts/facets/ERC1155PortalFacet.sol.
ERC1155_TRANSFER_HEADER =  b'Y\xda*\x98N\x16Z\xe4H|\x99\xe5\xd1\xdc\xa7\xe0L\x8a\x990\x1b\xe6\xbc\t)2\xcb]\x7f\x03Cx'  # TODO
# Function selector to be called during the execution of a voucher that transfers funds,
# which corresponds to the first 4 bytes of the Keccak256-encoded result of "safeBatchTransferFrom(address,address,uint256[],uint256[],bytes)",
TRANSFER_FUNCTION_SELECTOR = b'\xa9\x05\x9c\xbb'  # TODO
...
def handle_advance(data):
    ...
    try:
        ...
        # Attempt to decode input as an ABI-encoded ERC-1155 deposit
        binary = bytes.fromhex(data["payload"][2:])
        try:
            decoded = decode_abi(['bytes32', 'address', 'address', 'uint256', 'bytes'], binary)
        except Exception as e:
            msg = "Payload does not conform to ERC-1155 deposit ABI"
            logger.error(f"{msg}\n{traceback.format_exc()}")
            return reject_input(msg, data["payload"])

        # Check if the header matches the Keccak256-encoded string "ERC1155_Transfer"
        input_header = decoded[0]
        if input_header != ERC1155_TRANSFER_HEADER:
            return reject_input(f"Input header is not from an ERC-1155 transfer", data["payload"])

        # Post notice about the deposit
        depositor = decoded[1]
        erc1155 = decoded[2]
        amount = decoded[3]
        notice_str = f"Deposit received from: {depositor}; ERC-1155: {erc1155}; Amount: {amount}"
        ...
        # Post voucher executing the transfer on the ERC-1155 contract: "I don't want your money"!
        voucher = {"address": erc1155, "payload": "0x" + transfer_payload.hex()}
